...
Domain = karacos.model.Parent.extend({
	...
	init: function(values) {
		logger.debug("Initializing new Domain with values : " + sys.inspect(values));
		this._super(values);
		this.domain = this;
	},
	map: {
	...
	client.keys("Domain:*:fqdn*", function(err, domainkeys){
		...
		if (domainkeys.length !== 0) {
			...
			client.mget(domainkeys, function(err, domainfqdns){
				...
				id = domainkeys[domainfqdns.indexOf(fqdn)].split(':')[1];
...
function get_by_name(name, callback) {
	...
	client.keys("Domain:*:dname", function(err, domainkeys){
		var id;
		logger.debug("get_by_name:replies Domain:*:dname : " + domainkeys.length + " found - " + domainkeys);
		if (domainkeys.length !== 0) {
			// at least one domain key in redis
			client.mget(domainkeys, function(err, domainnames){
				...
				if (domainnames.sort().indexOf(name) === -1) {
					...
					if (typeof callback !== "undefined")
						callback("No domain found with that name " + name,undefined);
					return;
				}
